...
from ..scripts.child_fatality_scrape import download_all_pdfs, list_files, restructure_alphabetical_dict, merge_dicts
...
def test_download_all_pdfs(mock_html_page, mock_pdf_content):
    ...
    with patch.object(requests, 'get') as mock_get:
        ...
        with patch.object(os.path, 'exists') as mock_exists, patch("builtins.open", new_callable=MagicMock):
            ...
            assert mock_get.call_args_list[0][0][0] == url  # First request is for the HTML page
            assert mock_get.call_args_list[1][0][0] == 'https://dcfs.nv.gov//uploadedFiles/dcfsnvgov/content/Programs/CWS/CPS/ChildFatalities/Clark/document1.pdf'  # Second request is for the first PDF
            assert mock_get.call_args_list[2][0][0] == 'https://dcfs.nv.gov//uploadedFiles/dcfsnvgov/content/Programs/CWS/CPS/ChildFatalities/Clark/document2.pdf'  # Third request is for the second PDF
...
def test_list_files(tmp_path):
    ...
    assert set(list_files(tmp_path)) == set(dummy_files)  # Test that the list_files function correctly lists out the files


def test_restructure_alphabetical_dict():

    input_dict = {'A': 'Date of the notification to the child welfare agency of the fatality/near fatality of a child:\n4/13/2023\n', 'B': 'Location of child at the time of death or near fatality (city/county):\nLas Vegas, Clark\n'}
    expected_output_dict = {'Date of the notification to the child welfare agency of the fatality/near fatality of a child': '4/13/2023', 'Location of child at the time of death or near fatality (city/county)': 'Las Vegas, Clark'}
